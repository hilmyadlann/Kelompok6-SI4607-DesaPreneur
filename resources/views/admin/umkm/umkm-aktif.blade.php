        ...
        <div class="w-64 bg-green-500 text-white h-screen p-4 fixed top-0 left-0">
            ...
            <nav>
                <a href="{{ route('admin.form-umkm.index') }}" class="flex items-center px-4 py-2 rounded mb-2 {{ request()->routeIs('admin.form-umkm.index') ? 'bg-green-600' : 'hover:bg-green-600' }}">
                    <img src="{{ asset('images/clipboard.png') }}" alt="Form Icon" class="w-5 h-5 mr-2"> Manajemen Form Pendaftaran UMKM
                </a>
                        ...
                        <tbody id="table-body-umkm-aktif" class="bg-white divide-y divide-gray-200">
                            ...
                            <tr id="umkm-row-{{ $index }}" class="umkm-row">
                                ...
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <a href="{{ route('admin.umkm.lihat', $umkm) }}" class="bg-blue-500 text-white px-4 py-1 rounded hover:bg-blue-600 block">Lihat UMKM</a>
                                    <button type="button" onclick="confirmDelete('{{ $umkm->id }}')" class="bg-red-500 text-white px-4 py-1 rounded hover:bg-red-600 mt-2">Hapus UMKM</button>
                                    <form id="delete-form-{{ $umkm->id }}" action="{{ route('admin.umkm.hapus', $umkm) }}" method="POST" class="hidden">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </td>
