...
def run(playwright):
    ...
    # Klik UMKM Aktif
    try:
        with page.expect_navigation(url="http://127.0.0.1:8000/admin/umkm-aktif", timeout=10000):
            page.click('a.flex.items-center.px-4.py-2.rounded.mb-2.hover\\:bg-green-600')
        assertions_passed += 1
        print("Berhasil klik 'UMKM Aktif' dan masuk halaman UMKM Aktif")
    except TimeoutError:
        duration = time.time() - start_time
        print("Gagal klik 'UMKM Aktif' atau pindah halaman")
        print(f"Test: 1 Failed ({assertions_passed} Assertions Passed)")
        print(f"Duration: {duration:.2f}s")
        browser.close()
        return
